...
def parse_args():
    ...
    parser.add_argument("-a", "--asset", default="msft",
                        help="Crypto Asset name msft/appl/tsla/nvda, it will"
...
def get_data(cache_dir:str, asset_name:str, timeframe:str, limit:int):
    stock_inst = StockUSMarket(cache_dir)
    if not stock_inst.init():
        LOG.error("Fail to create US stock instance")
        return None

    asset_inst = stock_inst.get_asset(asset_name)
